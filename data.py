...
class TransformSamplingSubTraj:
    ...
    def __call__(self, traj):
        ...
        padding_mask = torch.from_numpy(padding_mask).to(dtype=torch.float32)
...
def create_dataloader(
    ...
    if verbose >= 2:
        ...
        print(f"  • Sample size: {sample_size} sub-trajectories")
        print(f"  • Batch size: {batch_size}")
        print(f"  • Number of iterations: {num_iters}")
        print(f"  • Number of workers: {num_workers}")
    elif verbose >= 1:
        print(f"📦 Creating dataloader with {sample_size} samples ({num_iters} iters × {batch_size} batch)")
    
    sampling_ind = sample_trajs(trajectories, sample_size)
    
    if verbose >= 2:
        print(f"  • Sampled trajectory indices: {len(sampling_ind)} total")
